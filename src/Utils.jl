...
@doc raw"""
    function max_cov!(M, work)

Compute the maximum covariance between columns
distinct pairs of columns of `M`.

# Arguments
* `M::StridedMatrix`: A matrix of size `(n, m)`
* `work::AbstractArray`: Scratch space of length at least `n`.
        More scratch space will usually improve performance,
